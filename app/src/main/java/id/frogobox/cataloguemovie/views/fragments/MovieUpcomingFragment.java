...
import static id.frogobox.cataloguemovie.helpers.ConstantsVar.Constants.STATE_KEY_FRAGMENT_UPCOMING;
...
public class MovieUpcomingFragment extends Fragment {
    ...
    private ArrayList<Movie> mArrayMovieUp = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        RecyclerView mRecyclerView = rootView.findViewById(R.id.upRecyclerView);
        mProgressBar = rootView.findViewById(R.id.upProgressBar);
        // -----------------------------------------------------------------------------------------
        ...
        if (hasConnection()) {
            Call<Result> callResultUpcoming = apiService.getUpComingMovie(API_KEY);
            callResultUpcoming.enqueue(new Callback<Result>() {
                @Override
                public void onResponse(Call<Result> call, Response<Result> response) {
                    mArrayMovieUp.clear();
                    List<Movie> listPopular = response.body().getResults();
                    mArrayMovieUp.addAll(listPopular);
                    adapter.addItem(mArrayMovieUp);
                    // -----------------------------------------------------------------------------
    ...
    private void getSavedInstanceState(Bundle savedInstanceState) {
        if (savedInstanceState != null){
            mProgressBar.setVisibility(View.INVISIBLE);
            mArrayMovieUp = savedInstanceState.getParcelableArrayList(STATE_KEY_FRAGMENT_UPCOMING);
            adapter.addItem(mArrayMovieUp);
            loadLayoutState.setLayoutState();
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelableArrayList(STATE_KEY_FRAGMENT_UPCOMING, mArrayMovieUp);
